...
export default function AutoBroadcastsScreen() {
  ...
  const selectedSummary = (() => {
    ...
    const teams = Object.values(selectedTeams).reduce((acc, set) => acc + (set?.size || 0), 0);
  ...
  const persistSelections = useCallback(async () => {
    ...
    try {
      console.log('[AutoBroadcasts] Persist start', { bar, selectedCompetitions, selectedTeams });
      // Build payloads
      const compIds = Object.entries(selectedCompetitions)
        .filter(([, checked]) => checked)
        .map(([id]) => ({ bar_id: bar, competition_id: /^\d+$/.test(id) ? Number(id) : id }));

      const teamIds: { bar_id: string; team_id: string }[] = [];
      Object.values(selectedTeams).forEach((set) => {
        set?.forEach((tid) => teamIds.push({ bar_id: bar, team_id: tid }));
      });
    ...
    if (!teamsCache[key]) {
      ...
      try {
        ...
        const teamIds = Array.from(
          new Set(
            (matchRows ?? []).flatMap(r => [r.home_team_id, r.away_team_id]).filter(Boolean)
          )
        );
        let teams: Team[] = [];
        if (teamIds.length > 0) {
  ...
  const toggleCompetitionSelected = useCallback((compId: string | number) => {
    ...
    // If selecting league, clear team-specific selections for it
    setSelectedTeams(prev => {
      const copy = { ...prev };
      if (!(selectedCompetitions[String(compId)])) {
        delete copy[String(compId)];
      }
      return copy;
    });
  ...
  const toggleTeamSelected = useCallback((compId: string | number, teamId: string) => {
    const key = String(compId);
    setSelectedCompetitions(prev => ({ ...prev, [key]: false }));
    setSelectedTeams(prev => {
            ...
            {compChecked ? (
              ...
              <FlatList
                ...
                renderItem={({ item: team }) => {
                  ...
                  const isChecked = selectedTeams[key]?.has(team.id) ?? false;
                  return (
    ...
    <SafeAreaView style={styles.container} edges={["top","bottom"]}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          onPress={persistSelections}
        >
          <Ionicons name="settings-outline" size={20} color="#FFFFFF" />
          <Text style={styles.primaryButtonText}>Automatizar estos partidos</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
