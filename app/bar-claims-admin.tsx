...
export default function BarClaimsAdminScreen() {
  ...
  const fetchPendingClaims = useCallback(async () => {
    const { data, error } = await supabase
      .from('bar_claims')
      .select('id, bar_id, contact_name, contact_phone, created_at, bars(name, address, city)')
      .eq('status', 'pending')
  ...
  }, [fetchPendingClaims, loadAdminFlag]);

  useFocusEffect(
    useCallback(() => {
      if (isAdmin) fetchPendingClaims().catch(() => {});
    }, [fetchPendingClaims, isAdmin]),
  );

  const renderItem = ({ item }: { item: ClaimRow }) => (
    <TouchableOpacity
      activeOpacity={0.85}
      style={styles.card}
      onPress={() => router.push(`/bar-claims-admin/${item.id}` as any)}
    >
      <View style={styles.cardRow}>
        <View style={styles.iconWrap}>
          <Ionicons name="storefront-outline" size={20} color="#4F8CFF" />
        </View>
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        ...
        <View style={styles.header}>
          ...
          <AppText style={styles.headerTitle}>Solicitudes de Reclamo</AppText>
          <View style={{ width: 28 }} />
        </View>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <FlatList
        ...
        contentContainerStyle={claims.length ? styles.listContent : styles.listEmptyContent}
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            ...
            <AppText style={styles.emptyTitle}>No hay solicitudes pendientes</AppText>
            <AppText style={styles.emptySub}>Cuando un propietario reclame un bar, aparecerá aquí.</AppText>
          </View>
        }
      />
    </SafeAreaView>
