  ...
  useEffect(() => {
    ...
    async function fetchOfferings() {
      try {
        ...
        if (!current) {
          if (isMounted) setError(new Error('No hi ha ofertes disponibles'));
          return;
        }
        ...
        if (isMounted) {
          setError(err instanceof Error ? err : new Error('Error carregant productes'));
        }
      } finally {
  ...
  const purchaseBoost = useCallback(async (
    ...
    try {
      ...
      if (insertError) {
        ...
        toast.warning('Boost comprat, però no s\'ha pogut registrar. Contacta amb suport.');
        return true;
      }

      toast.success('Boost activat!', 'El teu bar ara apareix destacat al mapa');
      return true;
    } catch (err: any) {
      ...
      toast.error('Error en la compra', 'Torna-ho a intentar');
      return false;
    } finally {
